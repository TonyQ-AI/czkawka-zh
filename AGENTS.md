...
本仓库启用 Spec Kit（spec-driven development）流程，遵循以下约定：

- 新功能开发统一走完整流程：`$speckit-constitution` → `$speckit-specify` → `$speckit-plan` → `$speckit-tasks` → `$speckit-implement` → `$speckit-converge`
- 规格/计划/任务产物统一存放在 `specs/<编号>-<功能名>/` 目录（spec.md / plan.md / tasks.md）
- 本仓库**不**维护 `docs/superpowers/` 下的规格产物，也不调用 brainstorming / writing-plans / executing-plans / domain-modeling 等本机流程技能——完整流程一律走 `$speckit-*`
- 执行质量纪律（TDD、系统化调试、验证后才宣布完成、代码审查）由本机技能自动承担，与 spec-kit 流程正交共存
