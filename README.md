...
> 基于 [qarmin/czkawka](https://github.com/qarmin/czkawka) 12.0.1 定制的**中文重复/相似照片筛查工具**，面向个人照片库去重。

**核心增强：自动建议保留最清晰照片。** 相似图片结果按清晰度自动排序（分辨率优先、同分辨率看大小），每组 ⭐ 标记最清晰的一张，可一键采纳或手动覆盖，确认后安全清理（默认回收站）。

**其他定制：** 完整中文界面 · 按钮悬浮提示 · 定制应用图标 · 隐藏对中文不适用的「不规范名称」工具 · 修复「PNG 内容 + jpg 后缀」文件被误报损坏的问题 · 「重复文件」默认扫描所有大小，不再跳过小文件（原版默认跳过 16KB 以下文件）。

👉 详细使用说明见 [`docs/zh/使用说明.md`](docs/zh/使用说明.md)
👉 功能规格与设计文档见 [`specs/001-duplicate-photo-tool/`](specs/001-duplicate-photo-tool/)
