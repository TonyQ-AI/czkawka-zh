...
#[test]
fn test_valid_png_content_with_jpg_extension_not_reported() {
    // A file whose content is a valid PNG but carries a .jpg extension must NOT be
    // flagged as broken: the check guesses the format from content, not the extension.
    let temp_dir = TempDir::new().unwrap();
    let img = image::ImageBuffer::from_pixel(64, 48, image::Rgba([10u8, 200, 30, 255]));
    img.save(temp_dir.path().join("real.png")).unwrap();
    fs::rename(temp_dir.path().join("real.png"), temp_dir.path().join("png_content_named_jpg.jpg")).unwrap();

    let results = run_check(&temp_dir, CheckedTypes::IMAGE);
    assert_eq!(results.len(), 0, "Valid PNG with .jpg extension should not be reported as broken");
}

#[test]
fn test_truncated_png_still_reported_as_broken() {
    // A genuinely broken file (valid PNG header but truncated data) must still be reported.
    let temp_dir = TempDir::new().unwrap();
    let broken_path = temp_dir.path().join("truncated.png");
    fs::write(&broken_path, b"\x89PNG\r\n\x1a\n\x00\x00\x00\x0dIHDRTRUNCATEDGARBAGE").unwrap();

    let results = run_check(&temp_dir, CheckedTypes::IMAGE);
    assert_eq!(results.len(), 1, "Truncated PNG should be reported as broken");
    assert!(results[0].has_errors());
}
