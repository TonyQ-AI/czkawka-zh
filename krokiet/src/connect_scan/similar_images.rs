    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            ...
            for (_first_entry, vec_fe) in &mut vector {
                // Sort each group by clarity (resolution first, then size) so the
                // sharpest photo ranks first and is suggested for keeping.
                czkawka_core::tools::similar_images::rank_group_by_clarity(vec_fe);
            }
...
fn write_similar_images_results(
    ...
    for (ref_fe, vec_fe) in vector {
        if let Some(ref_fe) = ref_fe {
            let (data_model_str, data_model_int) = prepare_data_model_similar_images(ref_fe, hash_size, false);
        ...
        for (idx, fe) in vec_fe.into_iter().enumerate() {
            // First member of each group is the sharpest photo, suggested for keeping.
            let suggested_to_keep = idx == 0;
            let (data_model_str, data_model_int) = prepare_data_model_similar_images(fe, hash_size, suggested_to_keep);
...
fn prepare_data_model_similar_images(fe: ImagesEntry, hash_size: u8, suggested_to_keep: bool) -> (ModelRc<SharedString>, ModelRc<i32>) {
    let (directory, file) = split_path(fe.get_path());
    let keep_marker = if suggested_to_keep { "⭐" } else { "" };
