...
fn extract_comparable_field(model: &SingleMainListModel, property: Property, active_tab: ActiveTab) -> u64 {
    ...
    match property {
        ...
        Property::Resolution => {
            let pixel_count = val_ints.nth(active_tab.get_int_pixel_count_idx()).expect("can find pixel count property") as u64;
            // 清晰度评分：分辨率优先，文件大小（KiB）次之——与 czkawka_core 的
            // clarity_score 保持一致，保证"保留最清晰"的选择结果可复现。
            let size_kib = {
                let mut ints = model.val_int.iter();
                let high = ints.nth(active_tab.get_int_size_idx()).expect("can find file size property");
                let low = ints.next().expect("can find file size property");
                connect_i32_into_u64(high, low) / 1024
            };
            pixel_count * 1000 + size_kib
        }
    }
}
